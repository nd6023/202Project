...
{ 
 ...
 public void sort(int[] values,int p,int q)
          ...
          for(i=p; i < values.length-1; i++){
            ...
            for(j=q; j < values.length-i; j++){
                if(values[j-1] > values[j]){
                    ...
                    getWorld().addObject(new Label(swap,15),700,y3+65);
                }
                ...
                setJ(j);
                setValues(values);
                getWorld().addObject(new Label("values: I="+i+" J="+j,15),700,y2-25);
                return;
            }
  ...
  setValues(values);
  y2=y3+65;
  getWorld().addObject(new Label("sorting done",25),350,y2); 
  return  ;
}
